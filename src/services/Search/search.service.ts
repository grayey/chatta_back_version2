...
export class SearchEngineService {
  ...
  search = async sentence => {
    ...
    if (!result) {
      ...
      else result = await this.findById('empty', null);
    }
    return result;
  };
  findById = (id, tree) => {
  ...
  buildResult = async rawResult => {
    ...
    if (rawResult.duplicateExactButtonMatch.length > 1) {
      ...
      const output = await this.findById(result.buttonKey, null);
      const fallback = await this.findById('empty', null);

      if (!output && (result.buttonValue.length || result.matchedTag)) {
        const prompt = `Sorry, we currently do not have the option "${result.buttonValue ||
          result.matchedTag}" available. We are working to make sure we include this option in the list of our services. Meanwhile you might want to checkout the services below`;
        const convo = {
      ...
      return await this.findById(result.buttonKey, null);
      ...
      return await this.findById(rawResult.exactKeywordResult.buttonKey, null);
    }
  };
  getParent = buttonKey => {
    const matchedButton = [];
